...
function modifyHand(str:string){
    let allLetters:string[] = ['A', 'K','Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2']
    let maxRank = 0;
    let tempstring =''
    for(let ch of allLetters){
        let temp:string = str;
        temp = temp.replace(/J/g, ch)
        let rank = findRank(temp)
        if(rank > maxRank){
            maxRank = rank;
            tempstring = temp;
        }
    ...
    console.log(tempstring, maxRank)
