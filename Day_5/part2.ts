...
function extractInputs(){
    let seeds:number[][] = [];
    let temp = content.substring(content.indexOf(':') + 1, content.indexOf('\n')).trim().split(" ").map((Number))

    for(let i = 0; i<temp.length; i+= 2){
       seeds.push([temp[i], temp[i+1]])
    }
    console.log(seeds)
    ...
    for(let seed of seeds){
        ...
        let limit = seed[0] + seed[1];
        let temp = seed[0];
        while(temp < limit){
            let soilNumber = findMapping(soil, temp)
...
function findMapping(array:number[][], num:number){
    for(let i = 1; i<array.length;i++){
        ...
        let limit = source + array[i][2];
        if(num >= source && num < limit){
            let d = num - source;
